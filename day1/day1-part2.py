...
def calibration(line):
    ...
    numbers_1_to_9 = list(map(lambda x: x + 1, list(range(9))))
    number_words = dict(
        zip(words, numbers_1_to_9)
    )

    matches = []
    
    groups = re.split(r'(\d)', line)
    for group in groups:
        try:
            matches.append(int(group))
            # print("got a number:", int(group))
        except ValueError as e: # substring, not an int literal
            # print("group:", group)
            ...
            while(end_index <= len(group)):
                ...
                # print(group)
                num_index = 0
                while num_index < len(number_words):
                    word = words[num_index]
                    number = numbers_1_to_9[num_index]
                    # print("considering substring:", group[start_index:end_index], " with word:", word)
                    if word in group[start_index:end_index]:
                        # print("matched a word:", word)
                        matches.append(number)
                        num_index = 0
                        # print("matches updated with ", number)
